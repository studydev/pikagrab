...
- 게임 종료 시 이름/점수가 Supabase 랭킹 테이블
;kuyikgh412;opygciiuwey에 저장됨
...
- game4: 2D 플랫폼 점프/흡수/변신 게임 (커비 스타일)
- game5: 브롤스타즈 스타일 탑다운 슈팅 게임
   - 다양한 적(일반, 강한, 빠른, 초강력, 공격형 느린 적)
   - 적마다 점수/HP/속도/공격력 다름
   - 거대 총알(관통, 적별 1회만 타격)
   - 모바일 가상 패드 지원
   - 맵에 낮은 확률로 케이크(HP회복) 등장, 1개만 존재, 먹으면 HP 2 회복
   - 점수/최고점수/HP UI, 게임오버/다시하기 버튼, 최고점수 파랑, 다시하기 버튼 하양
   - 코드 및 UI 지속 개선
